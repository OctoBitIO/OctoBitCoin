...
unsigned int nStakeMinAge = 24 * 60 * 60; // 24 hours as zero time weight
unsigned int nStakeMaxAge = 180 * nOneDay; // 180 days as full weight
unsigned int nStakeTargetSpacing = 5 * 60; // 1-minute stakes spacing
...
const string strMessageMagic = "OctobitCoin Signed Message:\n";
...
{
    return bnProofOfStakeLimit; // standart limit
...
int64_t GetProofOfWorkReward(unsigned int nBits, int64_t nFees)
{
    ...
    // OctobitCoin: subsidy is cut in half every 64x multiply of PoW difficulty
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, int64_t nTime, bool bCoinYearOnly)
{
    ...
    CBigNum bnRewardCoinYearLimit = MAX_MINT_PROOF_OF_STAKE; // Base stake mint rate, 40% year interest
    CBigNum bnTarget;
    bnTarget.SetCompact(nBits);
    ...
    CBigNum bnLowerBound = 1 * CENT, // Lower interest bound is 1% per year
        bnUpperBound = bnRewardCoinYearLimit, // Upper interest bound is 40% per year
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("octobitcoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in OctobitCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // OctobitCoin: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    int nMaxOffset = 76 * nOneHour; // 12 hours
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "OctobitCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const string strTimestamp = "Made By KsaAZaks && OctoBit.IO 10.03.2018";
        CTransaction txNew;
        txNew.nTime = 1520703109;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x0394e8713089da27f6b85ed79b45b9a973066857c6580534ab2feef1c492ad41"));
        block.print();
