...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("octobitcoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #octobitcoinTEST2\r");
            Send(hSocket, "WHO #octobitcoinTEST2\r");
        } else {
            // randomly join #octobitcoin00-#octobitcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #octobitcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #octobitcoin%02d\r", channel_number).c_str());
        }
